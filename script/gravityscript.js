...
//Boundaries.height=500;
var stage = [ Boundaries.x, Boundaries.y, Boundaries.width, Boundaries.height ];
...
function init() {
    ...
    worldAABB.maxVertex.Set( Boundaries.width + 200, Boundaries.height + 200 );
...
function getElementProperties( element ) {
    ...
    do {
        if(Boundaries.x>x+element.offsetLeft){
        ...
        if(Boundaries.y>y+element.offsetLeft){
            y=Boundaries.y;
        //            console.log('hi');
        }else{
            y += element.offsetTop;
...
function getBrowserDimensions() {
    ...
    if ( stage[0] != Boundaries.x ) {

        delta[0] = (Boundaries.x - stage[0]) * 50;
        stage[0] = Boundaries.x;
        changed = true;
    }

    if ( stage[1] != Boundaries.y ) {

        delta[1] = (Boundaries.y - stage[1]) * 50;
        stage[1] = Boundaries.y;
        changed = true;
    }

    if ( stage[2] != Boundaries.width ) {

        stage[2] = Boundaries.width;
        changed = true;
    }

    if ( stage[3] != Boundaries.height ) {

        stage[3] = Boundaries.height;
        changed = true;
    }
